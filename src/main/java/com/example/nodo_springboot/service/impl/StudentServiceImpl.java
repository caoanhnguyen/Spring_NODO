...
public class StudentServiceImpl implements StudentService {
    ...
    @Override
    @Transactional
    public ResponseData<?> addStudent(StudentRequestDTO studentRequestDTO) {
        HocSinh student = studentMapper.toEntity(studentRequestDTO);

        // Fetch Lop từ DB nếu có maLop trong DTO
        if (studentRequestDTO.getMaLop() != null && !studentRequestDTO.getMaLop().isBlank()) {
    ...
    @Override
    @Transactional
    public ResponseData<?> updateStudent(String maHS, StudentRequestDTO studentRequestDTO) {
        Optional<HocSinh> optionalHocSinh = studentRepository.findById(maHS);
        if (optionalHocSinh.isEmpty()) {
            return ResponseData.builder()
                    .data(null)
        ...
        HocSinh existingStudent = optionalHocSinh.get();
    ...
    @Override
    @Transactional
    public ResponseData<?> deleteStudent(String maHS) {
        Optional<HocSinh> optionalHocSinh = studentRepository.findById(maHS);
        if (optionalHocSinh.isEmpty()) {
            return ResponseData.builder()
                    .data(null)
        ...
        Lop lop = optionalHocSinh.get().getLop();

        studentRepository.delete(optionalHocSinh.get());

        List<?> hsList = lop.getHocSinhList();
